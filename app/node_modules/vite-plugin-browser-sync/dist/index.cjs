...
var import_browser_sync = require("browser-sync");
...
var Server = class {
  constructor(obj) {
    ...
    this.env = env;
    this.bsServer = (0, import_browser_sync.create)(this.name);
      ...
      this.bsServer.init(this.bsOptions, (error, bs) => {
        if (error) {
          this.config.logger.error(
            (0, import_kolorist.red)(`[vite-plugin-browser-sync] ${error.name} ${error.message}`),
            { error }
          );
          reject(error);
        }
   ...
   */
  log() {
    const colorUrl = (url) => (0, import_kolorist.lightYellow)(url.replace(/:(\d+)$/, (_, port) => `:${(0, import_kolorist.bold)(port)}/`));
      ...
      if (Object.prototype.hasOwnProperty.call(consoleTexts, key)) {
        ...
        if (Object.prototype.hasOwnProperty.call(urls, key)) {
          this.config.logger.info(
            `  ${(0, import_kolorist.lightYellow)("\u279C")}  ${(0, import_kolorist.bold)(
              `BrowserSync - ${text}`
            )}: ${colorUrl(urls[key])}`
          );
        }
  ...
  registerClose() {
    ...
    import_node_process.default.once("SIGINT", () => {
      this.bsServer.exit();
  ...
  return {
    ...
    apply(_config, env2) {
      ...
      if (options == null ? void 0 : options.bs) {
        console.error(
          (0, import_kolorist2.red)(
            `[vite-plugin-browser-sync] Since 3.0, you should wrap your ${(0, import_kolorist2.italic)("bs")} option inside a ${(0, import_kolorist2.italic)("dev")} object.`
          )
        );
        return false;
      }
